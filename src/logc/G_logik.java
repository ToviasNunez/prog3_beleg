...
public class G_logik  implements Serializable {
    ...
    public String getListProduzenten(List<?> itemsList) {
        List<Uploader> produzenten = new ArrayList<>();
        itemsList.stream().filter(v -> v instanceof InteractiveVideoImpl).map(v -> produzenten.add(((InteractiveVideoImpl) v).getUploader())).collect(Collectors.toList());
        itemsList.stream().filter(v -> v instanceof LicensedVideoImpl).map(v -> produzenten.add(((LicensedVideoImpl) v).getUploader())).collect(Collectors.toList());
        itemsList.stream().filter(v -> v instanceof LicensedAudioImpl).map(v -> produzenten.add(((LicensedAudioImpl) v).getUploader())).collect(Collectors.toList());
        itemsList.stream().filter(v -> v instanceof LicensedAudioVideoImpl).map(v -> produzenten.add(((LicensedAudioVideoImpl) v).getUploader())).collect(Collectors.toList());

        return countFrequnecies(produzenten);
    }
